...
export default function BaoCaoMarketing() {
  ...
  const fetchFirebaseReports = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://lumi-6dff7-default-rtdb.asia-southeast1.firebasedatabase.app/datasheet/B%C3%A1o_c%C3%A1o_MKT.json');
      ...
      if (response.ok) {
        ...
        const reportsArray = Object.entries(data).map(([id, report]) => ({
          ...
          revenue: report["Doanh số"] || report["DS chốt"] || report.revenue || 0,
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      const response = await fetch(`https://lumi-6dff7-default-rtdb.asia-southeast1.firebasedatabase.app/datasheet/B%C3%A1o_c%C3%A1o_MKT/${editingStatus.id}.json`, {
        method: 'PUT',
        headers: {
  ...
  const handleQuickStatusChange = async (reportId, newStatus) => {
    try {
      const response = await fetch(`https://lumi-6dff7-default-rtdb.asia-southeast1.firebasedatabase.app/datasheet/B%C3%A1o_c%C3%A1o_MKT/${reportId}.json`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://lumi-6dff7-default-rtdb.asia-southeast1.firebasedatabase.app/datasheet/B%C3%A1o_c%C3%A1o_MKT/${editingReport.id}.json`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDeleteReport = async () => {
    ...
    try {
      const response = await fetch(`https://lumi-6dff7-default-rtdb.asia-southeast1.firebasedatabase.app/datasheet/B%C3%A1o_c%C3%A1o_MKT/${deletingReport.id}.json`, {
        method: 'DELETE',
      });
  ...
  const totals = useMemo(() => {
    return filteredFirebaseReports.reduce((acc, report) => {
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="flex items-center justify-between mb-6 px-6 pt-6">
              <Link to="/" className="text-sm text-gray-600 hover:text-gray-800 flex-shrink-0 flex items-center gap-2">
                <ArrowLeft className="w-4 h-4" />
                Quay lại
              </Link>
              <h2 className="text-2xl font-bold text-primary uppercase text-center flex-1">Báo cáo Marketing</h2>
              <button
            ...
            {firebaseReports.length === 0 ? (
              ...
              <>
                <div className="mb-4 text-sm text-gray-600 px-6">
                  Hiển thị: <span className="font-semibold text-primary">{paginatedReports.length}</span> / {totalItems} báo cáo (Trang {currentPage} / {totalPages})
                </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 border border-gray-300">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      ...
                      <tr className="bg-green-700 font-semibold border-b-4 border-yellow-500">
                        <td colSpan="8" className="px-2 py-3 text-left pl-5 text-sm font-bold text-white border border-gray-300">
                          TỔNG CỘNG
                        </td>
                      ...
                      {paginatedReports.map((report, index) => {
                        ...
                        <tr key={report.id} className="hover:bg-gray-50">
                          <td className="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900 border border-gray-300">{actualIndex}</td>
